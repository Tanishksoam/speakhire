...
export function PublishFormDialog({ formId, formTitle, trigger, defaultOpen, open, onOpenChange }: PublishFormDialogProps) {
  ...
  const [publishForm, { isLoading, isSuccess, isError, error, data }] = usePublishFormMutation()
  
  // Update state based on the mutation status
  useEffect(() => {
    if (isSuccess && data) {
      console.log('Publish success, full response data:', JSON.stringify(data, null, 2))
      setIsPublished(true)
      ...
      try {
        ...
        if (data.form && typeof data.form === 'object') {
          ...
          if ('publishedUrl' in data.form && typeof data.form.publishedUrl === 'string') {
            console.log('Found publishedUrl in form object:', data.form.publishedUrl)
          ...
          if ('publishedUrl' in data && typeof data.publishedUrl === 'string') {
            console.log('Found publishedUrl directly in data:', data.publishedUrl)
  ...
  const handleSocialShare = (platform: string) => {
    ...
    switch (platform) {
      case "twitter":
        shareUrl = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareableLink)}&text=${encodeURIComponent(`Check out my form: ${formTitle}`)}`
        break
      case "facebook":
      ...
      case "linkedin":
        ...
        break
      case "email":
        shareUrl = `mailto:?subject=${encodeURIComponent(`Check out my form: ${formTitle}`)}&body=${encodeURIComponent(`I've created a form that I'd like to share with you: ${shareableLink}`)}`
        break
    }
  ...
  useEffect(() => {
    if (open && formId && !isPublished && !isPublishing) {
      // Automatically publish the form when the dialog opens
      handlePublish();
    }
  }, [open, formId, isPublished, isPublishing, handlePublish]);
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange} defaultOpen={defaultOpen}>
      {trigger && <DialogTrigger asChild>{trigger}</DialogTrigger>}
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Publish Form</DialogTitle>
          <DialogDescription>Make your form available to respondents and customize sharing settings.</DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="settings" className="space-y-4 pt-4">
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch id="response-limit" checked={responseLimitEnabled} onCheckedChange={setResponseLimitEnabled} />
              </div>

              {responseLimitEnabled && (
                <div className="pl-6 border-l-2 border-muted">
                  <Label htmlFor="max-responses">Maximum Responses</Label>
                  <Input id="max-responses" type="number" min="1" placeholder="100" className="mt-1" />
                </div>
              )}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Require a password to access the form.</p>
                </div>
                <Switch id="password-protection" checked={passwordEnabled} onCheckedChange={setPasswordEnabled} />
              </div>

              {passwordEnabled && (
                <div className="pl-6 border-l-2 border-muted">
                  <Label htmlFor="form-password">Form Password</Label>
                  <Input id="form-password" type="password" placeholder="Enter password" className="mt-1" />
                </div>
              )}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Receive an email when someone submits a response.</p>
                </div>
          ...
          {isPublished && shareableLink && (
            <div className="w-full p-4 bg-green-50 border border-green-200 rounded-md">
              <div className="flex items-center justify-between mb-2">
                <h4 className="font-medium text-green-800">Your form is now published!</h4>
                <Button 
                  variant="ghost" 
                  size="sm" 
